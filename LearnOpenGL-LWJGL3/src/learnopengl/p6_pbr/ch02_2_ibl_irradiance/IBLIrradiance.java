package learnopengl.p6_pbr.ch02_2_ibl_irradiance;
...
public class IBLIrradiance {
	...
	public static void main(String[] args) {
		...
		final String dir = IBLIrradiance.class.getResource(".").getFile();
		...
		try(MemoryStack stack = MemoryStack.stackPush()) {
			...
			IntBuffer maps = stack.mallocInt(2);
			
			createEnvironmentAndIrradianceMap(maps, cubeVAO);
	...
	private static void createEnvironmentAndIrradianceMap(IntBuffer outputMapsBuffer, int cubeVAO) {
		...
		bakeEnvironmentalMap(environmentMap, hdrTexture, captureFBO, equirectToCubemapShader, cubeVAO, captureProj, captureViews);

		final int irradianceMap = createIrradianceCubemap(environmentMap);

		bakeIrradianceMap(irradianceMap, environmentMap, captureFBO, captureRBO, irradianceShader, cubeVAO, 
		...
		outputMapsBuffer.put(0, environmentMap);
