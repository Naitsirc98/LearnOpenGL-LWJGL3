package learnopengl.p6_pbr.ch03_2_ibl_specular_textured;
...
public class IBLSpecularTextured {
	...
	public static void main(String[] args) {
		...
		final String dir = IBLSpecularTextured.class.getResource(".").getFile();
		...
		final Material rustedIron, gold, grass, plastic, wall;
		{			
			final String texDir = "resources/textures/pbr/";
			MaterialBuilder builder = new MaterialBuilder()
					.albedo("albedo.png")
					.normal("normal.png")
					.metallic("metallic.png")
					.roughness("roughness.png")
					.ambientOclussion("ao.png");
			
			rustedIron = builder.root(texDir+"rusted_iron/").build();
			gold = builder.root(texDir+"gold/").build();
			grass = builder.root(texDir+"grass/").build();
			plastic = builder.root(texDir+"plastic/").build();
			wall = builder.root(texDir+"wall/").build();
		}
		...
		while(!glfwWindowShouldClose(window)) {
			...
			rustedIron.bindTextures(3);
			model.translation(-5, 0, 2);
			pbrShader.setMat4("model", model);
			renderSphere(sphereVAO, indicesCount);
			
			gold.bindTextures(3);
			model.translation(-3, 0, 2);
			pbrShader.setMat4("model", model);
			renderSphere(sphereVAO, indicesCount);
			
			grass.bindTextures(3);
			model.translation(-1, 0, 2);
			pbrShader.setMat4("model", model);
			renderSphere(sphereVAO, indicesCount);
			
			plastic.bindTextures(3);
			model.translation(1, 0, 2);
			pbrShader.setMat4("model", model);
			renderSphere(sphereVAO, indicesCount);
	...
	// Class that groups all the PBR map textures for a certain material
	private static class Material {

		public final int albedo;
		public final int normal;
		public final int metallic;
		public final int roughness;
		public final int ambientOclussion;

		public Material(int albedo, int normal, int metallic, int roughness, int ambientOclussion) {
			this.albedo = albedo;
			this.normal = normal;
			this.metallic = metallic;
			this.roughness = roughness;
			this.ambientOclussion = ambientOclussion;
		}
		
		public void bindTextures(int startTextureUnit) {
			final int unit = GL_TEXTURE0 + startTextureUnit;
			...
			glActiveTexture(unit+3);
			glBindTexture(GL_TEXTURE_2D, roughness);
			
			glActiveTexture(unit+4);
			glBindTexture(GL_TEXTURE_2D, ambientOclussion);
		...
		public void delete() {
			...
			glDeleteTextures(ambientOclussion);
		}

	}

	// Class that helps with the material creation
	private static class MaterialBuilder {
		...
		private String ambientOclussion;
		
		public MaterialBuilder root(String path) {
			rootDir = path;
			return this;
		}

		public MaterialBuilder albedo(String path) {
			albedo = path;
			return this;
		}

		public MaterialBuilder normal(String path) {
			normal = path;
			return this;
		}

		public MaterialBuilder metallic(String path) {
			metallic = path;
			return this;
		}

		public MaterialBuilder roughness(String path) {
			roughness = path;
			return this;
		}

		public MaterialBuilder ambientOclussion(String path) {
			ambientOclussion = path;
			return this;
		}

		public Material build() {
			return new Material(
					loadTexture(rootDir + albedo),
					loadTexture(rootDir + normal),
					loadTexture(rootDir + metallic),
					loadTexture(rootDir + roughness),
					loadTexture(rootDir + ambientOclussion)
					);
		}
