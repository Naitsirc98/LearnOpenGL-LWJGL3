package learnopengl.p4_advanced_opengl.ch06_2_cubemap_environment_mapping;
...
public class CubemapsEnvironmentMapping {
	...
	public static void main(String[] args) {
		...
		final String dir = CubemapsEnvironmentMapping.class.getResource(".").getFile();
		Shader shader = new Shader(dir+"cubemaps.vs", dir+"cubemaps.fs");
		Shader skyboxShader = new Shader(dir+"skybox.vs", dir+"skybox.fs");
		...
		// Skybox VAO
		final int skyboxVAO = glGenVertexArrays();
		final int skyboxVBO = glGenBuffers();
		...
		final int cubemapTexture = loadCubemap(faces);
		...
		// Since we only use the skybox texture, bind it before render loop
		...
		while(!glfwWindowShouldClose(window)) {
			...
			// Draw skybox at last
			glDepthFunc(GL_LEQUAL); // Change depth function so depth test passes when values are less or equal to depth buffer's content

			skyboxShader.use();
			camera.getViewMatrix().get3x3(new Matrix3f()).get(view); // Remove translation from the view matrix
	 ...
	 * -Z (back) 
	 * */
	private static int loadCubemap(String[] faces) {
		...
		try(MemoryStack stack = MemoryStack.stackPush()) {
			...
			for(int i = 0;i < faces.length;i++) {
				...
				if(data != null) {
					...
					glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, format, width.get(0), height.get(0),
							0, format, GL_UNSIGNED_BYTE, data);					

				} else {
					throw new RuntimeException("Cubmap texture failed to load at path: " + faces[i]);
				}
